...
from model.title_long_summarization.util import load_embeddings
...
class SummarizationModel(object):
    ...
    def __init__(self, hps, vocab, num_gpus, num_iter=2):
        self._hps = hps
        self._cur_gpu = 0
        self._vocab = vocab
    ...
    def _add_variables(self):
        with tf.variable_scope('variables', initializer=tf.random_normal_initializer(mean=0, stddev=0.22, dtype=tf.float32)):
            self._recursive_weights = tf.get_variable(
                'recursive_weights', [self._hps.hidden_dim*2, self._hps.hidden_dim*2], dtype=tf.float32)
            self._recursive_bias = tf.get_variable(
                'recursive_bias', [self._hps.hidden_dim*2], dtype=tf.float32)

            tf.summary.histogram('recursive_weights',self._recursive_weights)
    ...
    def _add_placeholders(self):
        ...
        self._title_batch = tf.placeholder(
            tf.int32, [hps.batch_size, hps.max_title_len], name='title_batch')
            ...
            if self._hps.hier:
                with tf.variable_scope('encoder'), tf.device(self._next_device()):
                    ...
                    with tf.variable_scope("word_level_encoder", reuse=tf.AUTO_REUSE) as scope:
                        ...
                        for i in range(self._hps.num_sections):
                            encoded = encoder_outputs_words[i]
                            for j in range(self.num_iter):
                                _encoded = tf.matmul(
                                    encoded, self._recursive_weights)+self._recursive_bias
                                _title = tf.matmul(
                                    encoder_title_output, self._recursive_weights)+self._recursive_bias

                                title_attn = tf.matmul(
                                    _encoded, tf.transpose(_title, perm=[0, 2, 1]))
                                title_attn = tf.math.reduce_max(
                                    title_attn, axis=2)
                                title_attn = tf.nn.softmax(title_attn)
                                encoded = tf.expand_dims(
                                    title_attn, -1)*encoded
                            recursive_output.append(encoded)

                    # level 2, encode sections to doc
                    # shape [batch x num_sections x seq_len x hidden_size]
                    encoder_outputs_words = tf.stack(
                        recursive_output, axis=1)
