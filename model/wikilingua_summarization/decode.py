...
import model.wikilingua_summarization.beam_search as beam_search
import model.wikilingua_summarization.data as data
import json
import pyrouge
...
class BeamSearchDecoder(object):
    ...
    def __init__(self, model, batcher, vocab):
        ...
        ckpt_path = util.load_ckpt(
            self._saver, self._sess, latest_filename=decode_checkpoint)
    ...
    def decode(self):
        ...
        while True:
            ...
            files = os.listdir('/vinai/thongnt66/Repos/recursive-long-summarization/checkpoint/long_summarization/logroot/loss-experiment/decode_test_2500maxenc_4beam_35mindec_210maxdec_ckpt-32347/decoded')
            file_name=article_id+'_decoded.txt'
...
#       import pdb; pdb.set_trace()
            ...
            if FLAGS.single_pass:
                ...
                self.write_for_rouge(
                    original_abstract_sents, decoded_words, article_id)
                print_results(article_withunks, abstract_withunks,
                              decoded_output, article_id)  # log output to screen
                all_decoded[article_id] = self.prepare_for_attnvis(
                    article_withunks, abstract_withunks, decoded_words, best_hyp.attn_dists, best_hyp.p_gens, best_hyp.attn_dists_sec)
                counter += 1  # this is how many examples we've decoded
                self.write_for_attnvis(article_withunks, abstract_withunks, decoded_words, best_hyp.attn_dists,
                                       best_hyp.p_gens, best_hyp.attn_dists_sec)  # write info to .json file for visualization tool
            else:
                print_results(article_withunks, abstract_withunks,
                              decoded_output, article_id)  # log output to screen
                self.write_for_attnvis(article_withunks, abstract_withunks, decoded_words, best_hyp.attn_dists,
    ...
    def write_for_rouge(self, reference_sents, decoded_words, ex_index):
        ...
        Args:
          ...
          ex_index: str, the index with which to label the files
        """
        ...
        ref_file = os.path.join(self._rouge_ref_dir,
                                "%s_reference.txt" % ex_index)
        decoded_file = os.path.join(
            self._rouge_dec_dir, "%s_decoded.txt" % ex_index)

        with open(ref_file, "wb") as f:
            for idx, sent in enumerate(reference_sents):
                f.write(sent.encode('utf-8')) if idx == len(reference_sents) - \
                    1 else f.write((sent+"\n").encode('utf-8'))
        with open(decoded_file, "wb") as f:
            for idx, sent in enumerate(decoded_sents):
                f.write(sent.encode('utf-8')) if idx == len(decoded_sents) - \
                    1 else f.write((sent+"\n").encode('utf-8'))

        tf.logging.info("Wrote example %s to file" % ex_index)

    def prepare_for_attnvis(self, article, abstract, decoded_words, attn_dists, p_gens, attn_dists_sec=None):
        """Returns an object that will be written to file for attention visualization
        ...
        Args:
          ...
          attn_dists_sec: List of arrays; the attention distributions over sections (only in the hierarchical model).
        """
        ...
        if attn_dists_sec is not None:
            to_write['attn_dists_sec'] = attn_dists_sec
        if FLAGS.pointer_gen:
            to_write['p_gens'] = p_gens
        return to_write

    def write_all_for_attnvis(self, to_write_dict):
        output_fname = os.path.join(self._decode_dir, 'attn_vis_data_all.json')
        with open(output_fname, 'w') as output_file:
            json.dump(to_write_dict, output_file)
        tf.logging.info('Wrote visualization data to %s', output_fname)

    def write_for_attnvis(self, article, abstract, decoded_words, attn_dists, p_gens, attn_dists_sec=None):
...
def print_results(article, abstract, decoded_output, article_id):
    """Prints the article, the reference summmary and the decoded summary to screen"""
    print("")
#   tf.logging.info('ARTICLE:  %s', article)
    tf.logging.info('ARTICLE ID: %s', article_id)
...
def rouge_log(results_dict, dir_to_write):
    ...
    for x in ["1", "2", "3", "l"]:
        ...
        for y in ["f_score", "recall", "precision"]:
            ...
            header.append('RG-%s %s' % (x, y))
            rows.append('%.4f (%.4f, %.4f)' % (val, val_cb, val_ce))
            log_str += "%s: %.4f with confidence interval (%.4f, %.4f)\n" % (
                key, val, val_cb, val_ce)
    print(log_str)  # log to screen
    print('---\n')
    table = tabulate.tabulate([rows], headers=header, tablefmt='latex')
    df = pd.DataFrame([rows], columns=header)

    def filter(row):
        res = []
        for a in row:
            if isinstance(a, str) and re.search(r'0\.\d+', a):
                parts = re.findall('0\.\d+', a)
                res.append('{} ({}, {})'.format(
                    *[np.round(float(p) * 100, 4) for p in parts]))
            else:
                res.append(a)
        return res
    ...
    results_csv = os.path.join(dir_to_write, "ROUGE_results.csv")
    results_file_tbl = os.path.join(dir_to_write, "ROUGE_results_tbl.txt")
    tf.logging.info("Writing final ROUGE results to %s...", dir_to_write)
    with open(results_file, "w") as f:
        f.write(log_str)
    with open(results_file_tbl, "w") as f:
        f.write(table)
