...
def write_to_bin(infile, outfile, vocab_file=0, title=0):
    ...
    with open(infile) as f:
        ...
        for line in f:
            ...
            tf_example.features.feature['abstract'].bytes_list.value.extend(
                [abstract_str.encode('utf-8', 'ignore')])

            sections_str = _nested_list_to_string(data['sections'])
            tf_example.features.feature['sections'].bytes_list.value.extend(
                [sections_str.encode('utf-8', 'ignore')])

            # add title
            if title:
                title = data['title'].lower()
                tf_example.features.feature['title'].bytes_list.value.extend([title.encode('utf-8','ignore')])
            ...
            tf_example.features.feature['article_body'].bytes_list.value.extend(
                [article_body_str.encode('utf-8', 'ignore')])
            ...
            tf_example.features.feature['section_names'].bytes_list.value.extend(
                [section_names.encode('utf-8', 'ignore')])

            labels = _list_to_string(
                ['0' for _ in range(len(data['section_names']))])
            tf_example.features.feature['labels'].bytes_list.value.extend(
                [labels.encode('utf-8', 'ignore')])
            ...
            if vocab_file:
                ...
                if title:
                    tokens = art_tokens + abs_tokens + title_tokens
                else:
