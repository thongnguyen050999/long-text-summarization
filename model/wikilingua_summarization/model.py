...
from model.wikilingua_summarization.util import load_embeddings
...
class SummarizationModel(object):
    ...
    def __init__(self, hps, vocab, num_gpus):
        ...
        if FLAGS.new_attention and FLAGS.hier:
            ...
            from model.long_summarization.attention_decoder_new import attention_decoder
        else:
            print('using hierarchical attention mechanism for considering sections')
            from model.long_summarization.attention_decoder import attention_decoder
...
#         log_dists = [tf.log(dist) for dist in final_dists]
            ...
            if self._hps.mode in ['train', 'eval']:
                ...
                with tf.variable_scope('loss'), tf.device(self._next_device()):
                    ...
                    self.reg = add_degeneration_reg(self.embedding)
                    self._loss += self.reg
...
def add_degeneration_reg(embedding, vsize=50000):
    reg = tf.matmul(embedding, tf.transpose(embedding))
    aug = tf.reduce_sum(reg)
    del reg
    aug = aug/(vsize**2)
    aug *= 0.1
    return aug
