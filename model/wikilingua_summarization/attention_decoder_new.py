...
def attention_decoder(decoder_inputs,
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        # encoder state size, if this line fails, it's because the attention length isn't defined
        enc_output_size = encoder_states.get_shape()[2].value

        hier = True if encoder_section_states is not None else False

        # Reshape encoder_states (need to insert a dim)
        # now is shape (batch_size, attn_len, 1, enc_output_size)
        ...
        # shape (batch_size,seq_len,1,attention_vec_size)
...
#                'encoder_features.shape = ')
        ...
        if hier:
            ...
            # assumes section lenghts are equal
            nwords_sec = num_words_section[0][0]
            shapes_sec = encoder_section_states.shape
            ones = tf.ones(
                [shapes_sec[0].value, shapes_sec[1].value, nwords_sec, shapes_sec[2].value])
            # shape [batch_size, num_secs, sec_len, hidden]
            tmp_states = tf.multiply(tf.expand_dims(
                encoder_section_states, axis=2), ones)
            tmp_states = tf.reshape(
                tmp_states, [shapes_sec[0].value, -1, shapes_sec[2].value])
            encoder_section_states = tmp_states
            ###

            encoder_section_states = tf.expand_dims(
                encoder_section_states, axis=2)
            W_h_s = variable_scope.get_variable(
                "W_h_s", [1, 1, enc_sec_output_size, attention_vec_size])
            encoder_section_features = nn_ops.conv2d(encoder_section_states, W_h_s, [
                                                     1, 1, 1, 1], "SAME")  # shape (batch_size,seq_len,1,attention_vec_size)
            ...
            with variable_scope.variable_scope("Attention"):
                ...
                def masked_attention(e, enc_padding_mask):
                    if enc_section_padding_mask is not None:
                        enc_padding_mask = tf.reshape(
                            enc_section_padding_mask, [batch_size, -1])
                        enc_padding_mask = tf.cast(
                            enc_padding_mask, tf.float32)
                ...
                if use_coverage and coverage is not None:  # non-first step of coverage
                    if not hier:
                        ...
                        with tf.variable_scope("attention_words_sections"):
                            ...
                            e = math_ops.reduce_sum(v * math_ops.tanh(encoder_features + decoder_features +
                                                                      encoder_section_features + coverage_features), [2, 3])  # shape (batch_size,seq_len)
                            attn_dist = masked_attention(e, enc_padding_mask)
                            # shape=(batch_size, seq_len,1,1)
                    ...
                    if hier:
                        with tf.variable_scope("attention_words_sections"):
                            ...
                            e = math_ops.reduce_sum(
                                v * math_ops.tanh(encoder_features + decoder_features + encoder_section_features), [2, 3])
                ...
                # shape (batch_size, enc_output_size).
                ...
                context_vector = array_ops.reshape(
                    context_vector, [-1, enc_output_size])
        ...
        context_vector.set_shape([None, enc_output_size])
