...
class Hypothesis(object):
  ...
  def __init__(self, tokens, log_probs, state, attn_dists, p_gens, coverage, attn_dists_sec=None):
    ...
    self.attn_dists_sec = attn_dists_sec

  def extend(self, token, log_prob, state, attn_dist, p_gen, coverage, attn_dist_sec=None):
    ...
    return Hypothesis(tokens = self.tokens + [token],
                      ...
                      coverage = coverage,
                      attn_dists_sec = self.attn_dists_sec + [attn_dist_sec])
...
def run_beam_search(sess, model, vocab, batch):
  ...
  # dec_in_state is a LSTMStateTuple of [batch_size, hidden_dim]
  ...
  hyps = [Hypothesis(tokens=[vocab.word2id(data.START_DECODING)],
                     ...
                     attn_dists_sec=[] # zero vector of length attention_length
                     ) for _ in xrange(FLAGS.beam_size)]
  results = [] # this will contain finished hypotheses (those that have emitted the [STOP] token)

  steps = 0
  while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
    print('  {} / {} steps decoded'.format(steps, FLAGS.max_dec_steps), end='\r')
    ...
    (topk_ids, topk_log_probs, new_states, attn_dists, p_gens, new_coverage, attn_dists_sec) = model.decode_onestep(sess=sess,
                        batch=batch,
                        latest_tokens=latest_tokens,
                        enc_states=enc_states,
                        dec_init_states=states,
                        prev_coverage=prev_coverage)
    if attn_dists_sec is None: # workaround for linear attention sections
      attn_dists_sec = attn_dists
