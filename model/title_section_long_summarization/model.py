...
from model.title_section_long_summarization.util import load_embeddings
...
class SummarizationModel(object):
    ...
    def __init__(self, hps, vocab, num_gpus, num_iter=1):
        self._hps = hps
        self._cur_gpu = 0
        self._vocab = vocab
        ...
        if FLAGS.new_attention and FLAGS.hier:
            ...
            from model.title_long_summarization.attention_decoder_new import attention_decoder
        else:
            print('using hierarchical attention mechanism for considering sections')
            from model.title_long_summarization.attention_decoder import attention_decoder
        self.attn_decoder = attention_decoder

        self.num_iter = num_iter

    def _add_variables(self):
        with tf.variable_scope('variables', initializer=tf.random_normal_initializer(mean=0, stddev=0.22, dtype=tf.float32)):
            self._recursive_weights_title = tf.get_variable(
                'recursive_weights_title', [self._hps.hidden_dim*2, self._hps.hidden_dim*2], dtype=tf.float32)
            self._recursive_weights_section = tf.get_variable(
                'recursive_weights_section', [self._hps.hidden_dim*2, self._hps.hidden_dim*2], dtype=tf.float32)
            self._recursive_bias = tf.get_variable(
                'recursive_bias', [self._hps.hidden_dim*2], dtype=tf.float32)

            tf.summary.histogram('recursive_weights_title',self._recursive_weights_title)
            tf.summary.histogram('recursive_weights_section',self._recursive_weights_section)
            tf.summary.histogram('recursive_bias',self._recursive_bias)
    ...
    def _add_placeholders(self):
        ...
        if self._hps.hier:
            self._enc_batch_sections = tf.placeholder(
                tf.int32, [hps.batch_size, hps.num_sections, None], name='enc_batch_sections')
            self._doc_sec_lens = tf.placeholder(
                tf.int32, [hps.batch_size])  # length of doc in num sections
            self._batch_sections_len = tf.placeholder(
                tf.int32, [hps.batch_size, hps.num_sections])
            self._enc_section_padding_mask = tf.placeholder(
                tf.int32, [hps.batch_size, hps.num_sections, None], name='enc_section_padding_mask')
        ...
        # title
        self._title_batch = tf.placeholder(
            tf.int32, [hps.batch_size, hps.title_len], name='title_batch')
        self._title_padding_mask = tf.placeholder(
            tf.int32, [hps.batch_size, hps.title_len], name='batch_title_len')
        self._batch_title_len = tf.placeholder(
            tf.int32, [hps.batch_size], name='batch_title_len')
    ...
    def _make_feed_dict(self, batch, just_enc=False):
        ...
        feed_dict = {}
        if self._hps.hier:
            # shape=[batch-size, num-sections, enc-seq-len]
            feed_dict[self._enc_batch_sections] = batch.batch_sections
            # length of sections in the entire batch (num sections). [[400, 400, 400, 400],[...]]
            feed_dict[self._batch_sections_len] = batch.batch_sections_len
        ...
        feed_dict[self._batch_title_len] = batch.batch_title_len
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                if hps.pretrained_embeddings:
                    word2vec = load_embeddings(
                        hps.embeddings_path, self._vocab.word2id, hps.rand_unif_init_mag)
                ...
                if self._hps.hier:
                    enc_batch_sections = tf.unstack(
                        self._enc_batch_sections, axis=1)
                ...
                emb_title_inputs = tf.nn.embedding_lookup(
                    self.embedding, self._title_batch)
            ...
            if self._hps.hier:
                with tf.variable_scope('encoder'), tf.device(self._next_device()):
                    ...
                    with tf.variable_scope("word_level_encoder", reuse=tf.AUTO_REUSE) as scope:
                        ...
                        if self._hps.use_do:  # DropOut
                            cell_fw = tf.contrib.rnn.DropoutWrapper(
                                cell_fw, output_keep_prob=1.0 - self._hps.do_prob)
                            cell_bw = tf.contrib.rnn.DropoutWrapper(
                                cell_bw, output_keep_prob=1.0 - self._hps.do_prob)
                        ...
                        encoder_title_output, (fw_st, bw_st) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs=emb_title_inputs, dtype=tf.float32,
                                                                                               sequence_length=self._batch_title_len[:], swap_memory=True, initial_state_bw=bw_st, initial_state_fw=fw_st)
                        encoder_title_output = tf.concat(
                            axis=2, values=encoder_title_output)
                    ...
                    # shape=[batch x (seq_len * num_sections) x hidden_size] (4 x 2500 x 512)
                    self.encoder_outputs_words = encoder_outputs_words
                    encoder_outputs_words = tf.reshape(
                        encoder_outputs_words, (shapes[0].value, -1, shapes[-1].value))

                    # [batch x num_sections x hidden_size]
                    doc_sections_h = tf.stack([s.h for s in states], axis=1)
                    # [batch x num_sections x hidden_size]
                    doc_sections_c = tf.stack([s.c for s in states], axis=1)
                        ...
                        if FLAGS.section_level_encoder == 'RNN':
                            ...
                            if self._hps.use_do:
                                cell_fw_1 = tf.contrib.rnn.DropoutWrapper(
                                    cell_fw_1, output_keep_prob=1.0 - self._hps.do_prob)
                                cell_bw_1 = tf.contrib.rnn.DropoutWrapper(
                                    cell_bw_1, output_keep_prob=1.0 - self._hps.do_prob)
                            encoder_output_sections, (fw_st_2, bw_st_2) = tf.nn.bidirectional_dynamic_rnn(
                                cell_fw_1, cell_bw_1, inputs=doc_sections_h, sequence_length=self._doc_sec_lens, dtype=tf.float32, swap_memory=True)
                            ...
                            for j in range(self.num_iter):
                                _encoded = tf.matmul(
                                    encoder_output_sections, self._recursive_weights_section)+self._recursive_bias
                                _title = tf.matmul(
                                    encoder_title_output, self._recursive_weights_title)+self._recursive_bias

                                # [batch_size x 5 x 15]
                                title_attn = tf.matmul(
                                    _encoded, tf.transpose(_title, perm=[0, 2, 1]))/np.sqrt(hps.emb_dim*4)

                                title_attn = title_attn * tf.expand_dims(tf.cast(self._title_padding_mask, dtype=tf.float32),1)
                                title_attn = tf.math.reduce_max(
                                    title_attn, axis=2)
                                title_attn = tf.nn.softmax(title_attn)
                                encoder_output_sections = tf.expand_dims(
                                    title_attn, -1)*encoder_output_sections
                            ######################################################################################################
                            ...
                            if FLAGS.section_level_encoder == 'AVG':  # average section cells
                                ...
                                raise AttributeError('FLAGS.section_level_encoder={} is not a valid option'.format(
                                    FLAGS.section_level_encoder))
                ...
                with tf.variable_scope('encoder'):
                    with tf.variable_scope('word_level_encoder'):
                        ...
                        cell_bw = tf.contrib.rnn.LSTMCell(
                            self._hps.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
                        (encoder_outputs, (fw_st, bw_st)) = tf.nn.bidirectional_dynamic_rnn(
                            cell_fw, cell_bw, inputs=emb_enc_inputs, dtype=tf.float32, sequence_length=self._enc_lens, swap_memory=True)
                        # concatenate the forwards and backwards states
                        encoder_outputs = tf.concat(
                            axis=2, values=encoder_outputs)
                ...
                for layer_i in xrange(self._hps.enc_layers):
                    with tf.variable_scope('encoder%d' % layer_i), tf.device(
                        ...
                        if self._hps.use_do:  # add dropout
                            ...
                            cell_bw = tf.contrib.rnn.DropoutWrapper(
                                cell_bw, output_keep_prob=1.0 - self._hps.do_prob)
                        emb_enc_inputs, (fw_st, bw_st) = tf.nn.bidirectional_dynamic_rnn(
                            cell_fw, cell_bw, inputs=emb_enc_inputs, dtype=tf.float32, sequence_length=self._enc_lens, swap_memory=True)
                        emb_enc_inputs = tf.concat(
                            axis=2, values=emb_enc_inputs)
            ...
            with tf.variable_scope('decoder'), tf.device(self._next_device()):
                ...
                if self._hps.hier:
                    decoder_outputs, self._dec_out_state, self.attn_dists, self.p_gens, self.coverage, self.attn_dists_sec = self.attn_decoder(emb_dec_inputs,
                                                                                                                                               self._dec_in_state,
                                                                                                                                               self._enc_states,
                                                                                                                                               cell,
                    ...
                    decoder_outputs, self._dec_out_state, self.attn_dists, self.p_gens, self.coverage, _ = self.attn_decoder(emb_dec_inputs,
                                                                                                                             self._dec_in_state,
                                                                                                                             self._enc_states,
                                                                                                                             cell,
            ...
            with tf.variable_scope('output_projection'), tf.device(self._next_device()):
                if self._hps.output_weight_sharing:
                    ...
                    w_proj = tf.get_variable('w_proj', [self._hps.emb_dim, self._hps.hidden_dim],
                                             dtype=tf.float32, initializer=self.trunc_norm_init)
                    # shape = [vsize, hps.hidden_dim]
                    w = tf.tanh(tf.transpose(
                        tf.matmul(self.embedding, w_proj)))
    ...
    #         w_t = tf.transpose(w)
                ...
                for i, output in enumerate(decoder_outputs):
                    ...
                    vocab_scores.append(tf.nn.xw_plus_b(output, w, b))
...
#         log_dists = [tf.log(dist) for dist in final_dists]
    ...
    def _add_train_op(self):
        ...
        gradients = tf.gradients(
            loss_to_minimize, tvars, aggregation_method=tf.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N)
    ...
    def run_train_step(self, sess, batch):
        ...
        if FLAGS.debug:
            ...
            sess = tf_debug.LocalCLIDebugWrapperSession(
                sess, dump_root=FLAGS.dump_root, ui_type=FLAGS.ui_type)
...
#       import pdb; pdb.set_trace()
    ...
    def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
        ...
        feed[self._title_batch] = batch.title_batch
        feed[self._title_padding_mask] = batch.title_padding_mask
        feed[self._batch_title_len] = batch.batch_title_len
        ...
        if self._hps.hier:
            # shape=[batch-size, num-sections, enc-seq-len]
            feed[self._enc_batch_sections] = batch.batch_sections
        ...
        new_states = [tf.contrib.rnn.LSTMStateTuple(
            results['states'].c[i, :], results['states'].h[i, :]) for i in xrange(beam_size)]
