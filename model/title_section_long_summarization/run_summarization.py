...
from model.title_section_long_summarization.data import Vocab
from model.title_section_long_summarization.batch_reader import Batcher
from model.title_section_long_summarization.model import SummarizationModel
from model.title_section_long_summarization.decode import BeamSearchDecoder
from tensorflow.python import debug as tf_debug
import model.title_section_long_summarization.util as util
...
tf.app.flags.DEFINE_string('title_key', 'title',
                           'tf.Example feature key for sections.')
...
tf.app.flags.DEFINE_float('max_abstract_len', 500,
                          'Discards articles with longer abstracts')
tf.app.flags.DEFINE_float('min_abstract_len', 50,
                          'Discards articles with short abstracts')
tf.app.flags.DEFINE_float('max_article_sents', 350,
                          'Discards articles with short abstracts')
tf.app.flags.DEFINE_integer('max_title_len', 15,
                            'Discard articles with a long title')
tf.app.flags.DEFINE_integer('title_len', 10,
                            'Discard articles with a long title')
tf.app.flags.DEFINE_integer('min_title_len', 5,
                            'Discard articles with a short title')

tf.app.flags.DEFINE_boolean('use_sections', False,
                            'use hierarchical encoding/decoding over sections')
...
tf.app.flags.DEFINE_integer('max_article_len', 2400,
                            'Maximum input article length')
tf.app.flags.DEFINE_integer(
    'max_intro_len', 400, 'Maximum introduction section length')
tf.app.flags.DEFINE_integer(
    'max_conclusion_len', 400, 'Maximum conclusion section length')
tf.app.flags.DEFINE_integer('max_intro_sents', 20,
                            'Maximum introduction section length')
tf.app.flags.DEFINE_integer('max_conclusion_sents',
                            20, 'Maximum conclusion section length')
tf.app.flags.DEFINE_integer('max_section_sents', 20,
                            'Maximum section length in sentences')
tf.app.flags.DEFINE_integer(
    'num_sections', 6, 'Maximum introduction section length')
tf.app.flags.DEFINE_boolean(
    'hier', False, 'Hierarchical model to utilize section information')
tf.app.flags.DEFINE_boolean(
    'phased_lstm', False, 'Hierarchical model to utilize section information')
tf.app.flags.DEFINE_boolean('output_weight_sharing', False,
                            'If True, the weights of the model are shared between embedding and output projection layer')
tf.app.flags.DEFINE_boolean(
    'use_do', False, 'If True, use drop out on lstm cells in the encoder')
tf.app.flags.DEFINE_float('do_prob', 0.2, 'Dropout probability in lstm cells')

tf.app.flags.DEFINE_boolean('pretrained_embeddings',
                            False, 'use pretrained embeddings')
tf.app.flags.DEFINE_string('embeddings_path', '',
                           'path to plain text embedding files')

tf.app.flags.DEFINE_boolean('pubmed', False, 'pubmed data')

tf.app.flags.DEFINE_string('optimizer', 'adagrad',
                           'optimizer can be `adagrad`, `adam` or `sgd`')
tf.app.flags.DEFINE_boolean('multi_layer_encoder',
                            False, 'whether encoder is a multilayer LSTM')
tf.app.flags.DEFINE_integer('enc_layers', 1, 'number of encoder layers')

tf.app.flags.DEFINE_boolean('debug', False, 'debug mode')
tf.app.flags.DEFINE_string(
    'ui_type', 'curses', "Command-line user interface type (curses | readline)")
tf.app.flags.DEFINE_string(
    'dump_root', "/vinai/thongnt66/Repos/recursive-long-summarization/logs", "Location for dumping tfdbg logs")
...
tf.app.flags.DEFINE_boolean(
    'split_intro', False, 'Split intro into first and last parts.')
tf.app.flags.DEFINE_float(
    'temperature', 0.0, 'simulating temperature for softmax on attn_words')

tf.app.flags.DEFINE_boolean(
    'legacy_encoder', True, 'Use the older encoder (for compatibility issues).')
tf.app.flags.DEFINE_boolean(
    'fixed_attn', False, 'Use the older decoder (for compatibility issues).')

tf.app.flags.DEFINE_boolean('new_attention', False,
                            'Use the linear attention mapping for sections.')

tf.app.flags.DEFINE_string('section_level_encoder', 'RNN',
                           'section level encoder type: can be `RNN`, `AVG` (average section states) or `FF` (feed forward) ')
tf.app.flags.DEFINE_boolean('convert_linear_to_hier_attn', False,
                            'convert the linear attention model to hierarchical attention model.')
tf.app.flags.DEFINE_string('custom_decode_name', None,
                           'pass custom name for decoder directory.')
tf.app.flags.DEFINE_string('decode_checkpoint', None,
                           'custom checkpoint to decode.')
tf.app.flags.DEFINE_string('restore_checkpoint_name',
                           None, 'custom checkpoint to decode.')
...
def setup_training(model, batcher):
    ...
    if FLAGS.debug:
        ...
        sess_context_manager.add_tensor_filter(
...
def main(unused_argv):
    ...
    hparam_list = ['mode', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm',
                   ...
                   'max_title_len', 'min_title_len', 'title_key', 'title_len',
    ...
    if hps.mode == 'train':
        ...
        model = SummarizationModel(
            decode_model_hps, vocab, num_gpus=FLAGS.num_gpus)
