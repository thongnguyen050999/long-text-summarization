...
import model.title_section_long_summarization.data as data
...
class Example(object):
    ...
    def __init__(self, article, abstract_sentences, article_id, sections, section_names, labels, vocab, hps, title):
        ...
        if hps.hier:
            ...
            try:
                # last two sentences in the intro
                intro_last = sections[beg_loc][-2:]
            except IndexError:
...
#             i += 1
            ...
            if not hps.split_intro:
                ...
                # flatten list of sents, get the string inside, count words
                intro_last_len = len(intro_last_words)

                discard_last = False
                if intro_last_len > hps.max_intro_len:
                    discard_last = True
                len_limit = hps.max_intro_len - \
                    intro_last_len if not discard_last else hps.max_intro_len
                ...
                intro_words = _get_section_words(
                    intro_first, len_limit, pad=False)
                ...
                try:
                    if intro_words[-1] != '.':
                        ...
                        intro_words = _pad_words(
                            intro_words, hps.max_intro_len)
                except IndexError:
                ...
                intro_words = _get_section_words(
                    intro_first, hps.max_intro_len, pad=True)

            try:
                conclusion_words = _get_section_words(
                    sections[end_loc - beg_loc - 1][:hps.max_conclusion_sents], hps.max_conclusion_len)
            except:
                import pdb
                pdb.set_trace()
...
#           self.enc_sec_len = sec_len # TODO: Check
        ...
        # title
        self.title_len = hps.title_len
        title_words = title.split()[:self.title_len]
        title_ids = [vocab.word2id(w) for w in title_words]
        self.title_input = title_ids[:self.title_len]
        self.title_len_nopad = len(self.title_input)
        ...
        if hps.pointer_gen:
            ...
            self.title_input_extend_vocab = data.title2ids(
                title_words, vocab, self.article_oovs)
    ...
    def pad_title_input(self, max_len, pad_id):
        ...
        if self.hps.pointer_gen:
            while len(self.title_input_extend_vocab) < max_len:
                self.title_input_extend_vocab.append(pad_id)
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list, hps):
        ...
        # sequence
        max_title_len = hps.title_len
        for ex in example_list:
            ex.pad_encoder_input(max_enc_seq_len, self.pad_id)
        # title
            ex.pad_title_input(max_title_len, self.pad_id)

        self.title_batch = np.zeros(
            (hps.batch_size, max_title_len), dtype=np.int32)
        self.title_padding_mask = np.zeros(
            (hps.batch_size, max_title_len), dtype=np.int32)
        self.batch_title_len_nopad = np.zeros((hps.batch_size), dtype=np.int32)
        self.batch_title_len = np.zeros((hps.batch_size), dtype=np.int32)
        self.title_batch_extend_vocab = np.zeros(
            (hps.batch_size, max_title_len), dtype=np.int32)

        for i, ex in enumerate(example_list):
            for j in range(ex.title_len_nopad):
                self.title_padding_mask[i][j] = 1
            self.batch_title_len[i] = ex.title_len
            self.batch_title_len_nopad[i] = ex.title_len_nopad
            self.title_batch[i, :] = ex.title_input[:]
            self.title_batch_extend_vocab[i,
                                          :] = ex.title_input_extend_vocab[:]
        ##################################################################################################
        ...
        if self._hps.hier:
            ...
            for ex in example_list:
                ...
                ex.pad_section_input(
                    self._hps.max_section_len, max_num_sections, self.pad_id)
            self.batch_doc_sec_lens = [max_num_sections for _ in example_list]
            self.batch_sections = np.zeros(
                (hps.batch_size, max_num_sections, self._hps.max_section_len), dtype=np.int32)
            self.batch_sections_len = np.zeros(
                (hps.batch_size, max_num_sections), dtype=np.int32)
            self.batch_sections_len_nopad = np.zeros(
                (hps.batch_size, max_num_sections_nopad), dtype=np.int32)
            self.enc_section_padding_mask = np.zeros(
                (hps.batch_size, max_num_sections, self._hps.max_section_len), dtype=np.float32)
            for i, ex in enumerate(example_list):
                j = 0
                while(j < len(ex.enc_sections)):
                    self.batch_sections[i, j,
                                        :] = ex.enc_sections[j][:self._hps.max_section_len]
                ...
                try:
                    self.batch_sections_len_nopad[i,
                                                  :] = ex.num_words_section_nopad[:]
                    ...
                    for k in range(len(ex.num_words_section_nopad)):
                        self.batch_sections_len_nopad[i,
                                                      k] = ex.num_words_section_nopad[k]
        ...
        while True:
            try:
                ...
                (article_id, article_text, abstract_sents, labels,
                 section_names, sections, title) = six.next(input_gen)
...
#       abstract_sentences = [sent.strip() for sent in data.abstract2sents(''.join(abstract_sents))]
            ...
            if len(title_words) > self._hps.max_title_len or len(title_words) < self._hps.min_title_len: continue
    ...
    def text_generator(self, example_gen):
        ...
        while True:
            ...
            try:
                ...
                title = self._get_example_feature(e, self._title_key)
            ...
            yield (article_id, article_text, abstract_text, labels, section_names, sections, title)
